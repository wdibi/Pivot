...
function areEqual(left, right) {
    return left instanceof NumericLiteral && right instanceof NumericLiteral && left.value === right.value;
}
...
BinaryExpression.prototype.optimize = function() {
    ...
    // Sub
    if (this.op === '-' && areEqual(this.left, this.right)) return new NumericLiteral(0);
    ...
    // Div
    if (this.op === '/' && areEqual(this.left, this.right)) return new NumericLiteral(1);

    // Mod
    if (this.op === '%' && areEqual(this.left, this.right)) return new NumericLiteral(0);

    // Less Than and Greater Than
    if ((this.op === '<' || this.op === '>') && areEqual(this.left, this.right)) return new  BooleanLiteral(false);

    // Equals
    if ((this.op === '<=' || this.op === '>=' || this.op === '==') && areEqual(this.left, this.right)) return new  BooleanLiteral(true);

    // And
    if (isAndOp(op) && (isFalse(this.left || isFalse(this.right)))) return new BooleanLiteral(false);

    // Or
    if (isOrOp(or) && (isTrue(this.left) || isTrue(this.right))) return new BooleanLiteral(true);
